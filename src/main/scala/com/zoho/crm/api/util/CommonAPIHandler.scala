...
class CommonAPIHandler {
  ...
  def apiCall[A](className :Class[A], encodeType :String) :Option[APIResponse[A]] = {
    ...
    try {
      connector.addHeader(Constants.ZOHO_SDK, System.getProperty("os.name") + "/" + System.getProperty("os.version") + " scala/" + util.Properties.versionNumberString + ":" + Constants.SDK_VERSION)
